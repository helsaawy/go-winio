...
func DialPipeAccess(ctx context.Context, path string, access uint32) (net.Conn, error) {
	...
	if flags&windows.PIPE_TYPE_MESSAGE != 0 {
		return &win32MessageBytePipe{
			win32Pipe: win32Pipe{File: f, path: path},
		}, nil
	}
	return &win32Pipe{File: f, path: path}, nil
}
...
func (l *win32PipeListener) makeServerPipe() (*fs.File, error) {
...
func (l *win32PipeListener) makeConnectedServerPipe() (*fs.File, error) {
...
func connectPipe(p *fs.File) error {
	c, err := p.PrepareIO()
	if err != nil {
		return err
	}
...
func (l *win32PipeListener) Accept() (net.Conn, error) {
	...
	case l.acceptCh <- ch:
		...
		if l.config.MessageMode {
			return &win32MessageBytePipe{
				win32Pipe: win32Pipe{File: response.f, path: l.path},
			}, nil
		}
		return &win32Pipe{File: response.f, path: l.path}, nil
	case <-l.doneCh:
